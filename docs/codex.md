...
当前版本也会识别 Codex 的 User-Agent，并把 `/v1/models` 转换为 Codex models manager 所需格式。`codex-catalog` 仍适合作为静态、可复现的模型快照：它会拉取当前账号可用模型（含真实上下文窗口、图片能力与已验证的并行工具能力），写入 `~/.codex/costrict-router-model-catalog.json`，并把 `~/.codex/config.toml` 的 `model_catalog_json` 指向它（首次会备份为 `config.toml.bak`）。
