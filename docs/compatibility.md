...
- **部分内置能力用不了**：联网搜索、文件检索、Computer Use 这类依赖上游专门支持的功能都不可用（CoStrict 网关只接受 `function` 类型工具，各家原生格式实测均被拒绝），只有你自己定义的函数工具能正常调用。三个入口（`/v1/chat/completions`、`/v1/responses`、`/v1/messages`）的处理一致：内置工具与函数工具混用时内置部分被剔除、其余正常转发；**请求里只有内置工具时会直接返回明确报错**——静默剔除会让模型在"被暗示用工具却没有工具"时把幻觉当结果输出（Claude Code 的 WebSearch 子请求就是这种形态，报错能让它如实提示搜索失败）。
