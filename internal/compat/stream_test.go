...
func TestTransformChatCompletionStreamToResponses(t *testing.T) {
	upstream := strings.NewReader("data: {\"choices\":[{\"delta\":{\"content\":\"hi\"}}]}\n\n" +
		...
		"data: [DONE]\n\n")
	body, err := io.ReadAll(TransformChatCompletionStreamToResponses(upstream))
	if err != nil {
		t.Fatal(err)
	}
	text := string(body)
	for _, want := range []string{"event: response.created", "event: response.output_item.added", "event: response.output_text.delta", "event: response.completed", `"input_tokens":3`} {
...
func TestTransformChatCompletionStreamToAnthropic(t *testing.T) {
	upstream := strings.NewReader("data: {\"choices\":[{\"delta\":{\"content\":\"hi\"}}]}\n\n" +
		...
		"data: [DONE]\n\n")
	body, err := io.ReadAll(TransformChatCompletionStreamToAnthropic(upstream))
	if err != nil {
		t.Fatal(err)
	}
	text := string(body)
	for _, want := range []string{"event: message_start", "event: content_block_start", "event: content_block_delta", "event: message_delta", "event: message_stop", `"input_tokens":3`} {
		if !strings.Contains(text, want) {
			t.Fatalf("missing %q in stream:\n%s", want, text)
		}
	}
